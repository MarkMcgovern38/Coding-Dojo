...
const EditRound = (props) => {
    ...
    useEffect(() => {
        if (!id) {
            console.error("Round ID not found");
            return;
        }

        axios.get(`http://localhost:8000/api/v1/rounds/${id}`, {
            ...
            .then((res) => {
                setName(res.data.courseName);
                setScores(res.data.scores);
                setNotes(res.data.notes || "");
            })
            .catch((err) => console.log("Error fetching round:", err));
    }, [id]);
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        if (!id) {
            console.error("Round ID is missing, cannot update.");
            return;
        }
        ...
        axios.put(`http://localhost:8000/api/v1/rounds/${id}`, updatedRound, {
            withCredentials: true,
            headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
            ...
            .catch((err) => {
                console.error("Error updating round:", err.response?.data?.errors || err.message);
            });
    };
